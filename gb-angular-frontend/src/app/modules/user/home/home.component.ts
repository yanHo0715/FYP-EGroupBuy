...
*/
  ...
  private setPageTitle(){
    this.title.setTitle('SmartGroup | Customer HomePage');
  }

  private setPageMeta() {
    this.meta.updateTag({ name: 'viewport', content: `width=device-width, initial-scale=1.0` });
    this.meta.updateTag({ name: 'title', content: `SmartGroup | Customer HomePage` });
    this.meta.updateTag({ name: 'author', content: `ColorlibHQ` });
    this.meta.updateTag({ name: 'description', content: `SmartGroup | Customer HomePage` });
